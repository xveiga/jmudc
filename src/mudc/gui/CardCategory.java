...
public class CardCategory extends JPanel {
	...
	private Color backgroundColor = Colors.cardCategoryBackground;
	private Color foregroundColor = Colors.black;
	private boolean drawShadow = false;
	private int shadowSize = 0;
	private int bottomSpacing = 0;

	public CardCategory(MoodleCategory category) {
		...
		descLabel = new JLabel(category.summary);
		if (category.summary != null && !category.summary.equals("")) {
			if (category.summaryformat == 1) {
				descLabel.setText("<HTML>" + category.summary + "</HTML>");
			}
			else {
				descLabel.setText(category.summary);
			}
			descLabel.setFont(new Font("Tahoma", Font.PLAIN, 10));
			//descLabel.setBorder(new MatteBorder(1, 1, 1, 1, new Color(224, 224, 224, 255)));
			springLayout.putConstraint(SpringLayout.NORTH, descLabel, 0, SpringLayout.SOUTH, nameLabel);
	...
	public int getTotalContentHeight() {
		return (int) descLabel.getPreferredSize().getHeight() + 26 + shadowSize + bottomSpacing;
	}
}
